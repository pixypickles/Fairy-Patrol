(() => {
  ...
  // v3.1: 外部音声ファイルに依存しない Web Audio 効果音。
  ...
  const SFX = (() => {
    ...
    function play(name) {
      ...
      switch (name) {
        ...
        case 'splash': noise(.18,{filter:1750,volume:.075,q:.55}); tone(310,.10,{to:190,type:'sine',volume:.055,filter:1200}); break;
        case 'mushroom': tone(185,.10,{to:420,type:'sine',volume:.105}); tone(120,.08,{to:210,type:'triangle',volume:.045,delay:.035}); break;
      }
    }
  ...
  function loadRoom(index, save = true) {
    ...
    // v3.1 環境反応。入口では進行方向に音の鳴るキノコを置き、風の部屋には水たまりとキノコを追加する。
    if (currentRoomIndex === 0) hazards.push({type:'mushroom',x:480,y:352,pulse:0,bounce:0,touchCooldown:0});
    if (hazards.some(h => h.type === 'wind')) {
      const variant = currentRoomIndex % 3;
      const px = variant === 0 ? 660 : (variant === 1 ? 535 : 390);
      const py = variant === 0 ? 350 : (variant === 1 ? 315 : 365);
      hazards.push({type:'puddle',x:px,y:py,w:92,h:42,pulse:Math.random()*6.28,ripple:0,splashCooldown:0,wasWindActive:false});
      hazards.push({type:'mushroom',x:px-68,y:py-8,pulse:Math.random()*6.28,bounce:0,touchCooldown:0,windLinked:true});
    }
  ...
  function updateHazards(dt) {
    ...
    for (const hazard of hazards) {
      ...
      if (hazard.type === 'spikes') {
        ...
        if (!hazard.active && hazard.timer >= (hazard.interval || 5)) { hazard.active=true; hazard.activeTimer=hazard.duration || 2; hazard.timer=0; SFX.play('wind'); messageEl.textContent='右から強風が吹いてきた！'; }
        if (hazard.active) {
        ...
        hazard.ripple = Math.max(0, (hazard.ripple || 0) - dt * 1.8);
        hazard.splashCooldown = Math.max(0, (hazard.splashCooldown || 0) - dt);
        const windNow = hazards.some(w => w.type === 'wind' && w.active &&
          hazard.x + hazard.w/2 > w.x && hazard.x + hazard.w/2 < w.x+w.w &&
          hazard.y + hazard.h/2 > w.y && hazard.y + hazard.h/2 < w.y+w.h);
        if (windNow && !hazard.wasWindActive) {
          hazard.ripple = 1; hazard.splashCooldown = .55; SFX.play('splash'); burst(hazard.x+hazard.w/2,hazard.y,10);
          const linked = hazards.find(m => m.type === 'mushroom' && m.windLinked && Math.hypot(m.x-(hazard.x-68),m.y-(hazard.y-8)) < 35);
          if (linked) { linked.bounce = 1; SFX.play('mushroom'); }
        } else if (windNow && hazard.splashCooldown <= 0) {
          hazard.ripple = 1; hazard.splashCooldown = .62; SFX.play('splash'); burst(hazard.x+Math.random()*hazard.w,hazard.y+Math.random()*hazard.h*.45,5);
        }
        hazard.wasWindActive = windNow;
        const inside = player.z < 16 && player.x > hazard.x && player.x < hazard.x+hazard.w && player.y > hazard.y && player.y < hazard.y+hazard.h;
        if (inside && hazard.touchCooldown <= 0) { hazard.ripple=1; hazard.touchCooldown=.3; SFX.play('splash'); }
        hazard.touchCooldown=Math.max(0,(hazard.touchCooldown||0)-dt);
      } else if (hazard.type === 'mushroom') {
        hazard.bounce = Math.max(0, (hazard.bounce || 0) - dt * 3.4);
        hazard.touchCooldown = Math.max(0, (hazard.touchCooldown || 0) - dt);
        if (player.z < 24 && Math.hypot(player.x-hazard.x,player.y-hazard.y) < player.radius+20 && hazard.touchCooldown <= 0) {
          hazard.bounce=1; hazard.touchCooldown=.5; SFX.play('mushroom');
        }
  ...
  function drawHazard(hazard) {
    ...
    if (hazard.type === 'puddle') {
      ctx.save();
      const cx=hazard.x+hazard.w/2, cy=hazard.y+hazard.h/2;
      ctx.globalAlpha=.78; ctx.fillStyle='#4da9bd'; ctx.strokeStyle='#244f5d'; ctx.lineWidth=3;
      ctx.beginPath(); ctx.ellipse(cx,cy,hazard.w/2,hazard.h/2,0,0,Math.PI*2); ctx.fill(); ctx.stroke();
      ctx.globalAlpha=.55; ctx.strokeStyle='#c9fbff'; ctx.lineWidth=2;
      const r=(hazard.ripple||0)*28 + 9 + Math.sin(hazard.pulse*2)*2;
      ctx.beginPath(); ctx.ellipse(cx,cy,r,r*.38,0,0,Math.PI*2); ctx.stroke();
      const windNow=hazards.some(w=>w.type==='wind'&&w.active);
      if(windNow){ctx.globalAlpha=.8;ctx.fillStyle='#d7ffff';for(let i=0;i<5;i++){const a=hazard.pulse*2+i;ctx.beginPath();ctx.arc(cx-25+((a*23)%55),cy-12-Math.abs(Math.sin(a))*13,2.5,0,Math.PI*2);ctx.fill();}}
      ctx.restore(); return;
    }
    if (hazard.type === 'mushroom') {
      ctx.save(); ctx.translate(hazard.x,hazard.y);
      const b=Math.sin((1-(hazard.bounce||0))*Math.PI*2)*10*(hazard.bounce||0);
      ctx.translate(0,-Math.abs(b)); ctx.scale(1+(hazard.bounce||0)*.18,1-(hazard.bounce||0)*.12);
      ctx.fillStyle='#ead9bd';ctx.strokeStyle='#172019';ctx.lineWidth=4;ctx.beginPath();ctx.roundRect(-7,-2,14,25,6);ctx.fill();ctx.stroke();
      ctx.fillStyle='#e16f7e';ctx.beginPath();ctx.arc(0,-5,20,Math.PI,0);ctx.lineTo(18,2);ctx.quadraticCurveTo(0,9,-18,2);ctx.closePath();ctx.fill();ctx.stroke();
      ctx.fillStyle='#fff0cf';for(const [x,y,r] of [[-8,-8,4],[7,-12,3],[1,-2,3]]){ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();}
      ctx.restore(); return;
    }
